...
export const updateProfileData = createAsyncThunk<ProfileType, string, ThunkConfigType<ProfileValidationErrors[]>>(
    ...
    async (profileId, thunkAPI) => {
        ...
        try {
            const { data } = await extra.api.put<ProfileType>(`/profile/${profileId}`, formData);
            if (!data) {
                throw new Error();
            }
            return data;
